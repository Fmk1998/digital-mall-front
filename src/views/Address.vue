<template>
    <div>
        <navheader/>
        <nav-bread><span slot="bread">提交订单</span></nav-bread>
            ...
            <div class="container">
                <div class="checkout-addr">
                    ...
                    <el-steps :active="active" finish-status="success">
                        ...
                        <el-step title="方式"></el-step>
                        <el-step title="确定"></el-step>
                    </el-steps>

                    <!-- address list -->
                    <div class="page-title-normal checkout-title" v-if="active===0">
                        <h2><span>收货地点</span></h2>
                        <div class="block">
                            <el-cascader
                                    placeholder="输入城市"
                                    :options="city"
                                    filterable v-model="orderdetails.place" size="medium" clearable></el-cascader>
                            <el-input
                                    class="citydetail"
                                    type="textarea"
                                    placeholder="请输入详细地址"
                    ...
                    <!-- payment method -->
                    <div class="page-title-normal checkout-title" v-if="active===1">
                        <h2><span>支付方式</span></h2>
                        <h3><span>目前只支持货到付款</span></h3>
                        <el-input
                                placeholder="请输入手机号"
                        ...
                        <div class="payment">
                            <el-popover
                                    ...
                                    width="300"
                                    trigger="hover">
                                <img width="270" src="/static/支付.jpg" alt="alibaba"/>
                                <el-button slot="reference">支付宝支付</el-button>
                            </el-popover>
                            <el-popover
                                    placement="top-start"
                                    title="请用微信扫码"
                                    ...
                                    trigger="hover">
                                <img width="270" src="/static/支付.jpg" alt="alibaba"/>
                                <el-button slot="reference">微信支付</el-button>
                            </el-popover>
                            <el-button @click="setpayment" slot="reference">货到付款</el-button>
                        </div>
                    </div>

                    <!-- payment method -->
                    <div class="page-title-normal checkout-title" v-if="active===2">
                        <h2><span>确定下单</span></h2>
                        地点：
                        <el-cascader
                                placeholder="输入城市"
                                :options="city"
                                filterable v-model="orderdetails.place" size="medium" clearable :disabled="true"></el-cascader>
                        <br/>
                        电话：
                        <el-input
                        ...
                        付款金额：
                        <el-input
                                :placeholder="orderdetails.totalprice"
                                :disabled="true">
                        </el-input>
                        <br/>
                        购买商品：
                        <el-input
                                :placeholder="orderitemsname"
                                :disabled="true">
                        </el-input>
                    </div>

                    <!-- payment method -->
                    <div class="page-title-normal checkout-title" v-if="active===3">
                        <h2><span>下单完成</span></h2>
                    </div>

                    <div class="next-btn-wrap">
                        <el-button v-if="this.active>0&&this.active<3" style="margin-top: 12px;" @click="pre">上一步</el-button>
                        <el-button v-if="this.active<2" style="margin-top: 12px;" @click="next">下一步</el-button>
                        <el-button v-if="this.active===2" style="margin-top: 12px;" @click="checkout">确定</el-button>
                        <el-button v-if="this.active===3" style="margin-top: 12px;" @click="goHome">返回首页</el-button>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        data() {
            ...
            return {
                active: 0,
                orderitems: this.$route.params.orderitems,
                orderitemsname: '',
                oids: [],
                citydetail: '',
                payment: '',
        ...
        methods: {
            ...
            open() {
                ...
                for (let i in this.orderitems) {
                    this.orderitemsname = this.orderitems[i].pname + ';' + this.orderitemsname;
                    this.oids.push(this.orderitems[i].oid);
                }
                ;
            },
            pre() {
            ...
            checkout() {
                ...
                if (this.orderdetails.place === "" || this.orderdetails.place.length === 0) return this.$message.error('你还没选择地址！');
                if (this.orderdetails.phone === "") return this.$message.error('你还没输入手机号！');
                if (this.payment === "") return this.$message.error('你还没选择付款方式！');
                this.orderdetails.time = new Date();
                this.orderdetails.odid = this.getUuid();
                this.orderdetails.place = this.orderdetails.place.join();
                console.log(this.orderdetails);
                this.addOrderDetail(this.orderdetails);
                for (let i in this.oids){
                    this.cartItemChange(this.oids[i], this.orderdetails.odid);
                }
            ...
            //添加订单详情
            addOrderDetail(item) {
                axios.post('/api/orderdetails', item).then((result) => {
                    console.log(result);
                })
            },
            //修改购物车信息
            cartItemChange(oid,odid) {
                let itemModel = {oid: oid,  ordered: 1, odid: odid};
            ...
            //生成uuid
            getUuid(){
                let all="0123456789azxcvbnmsdfghjklqwertyuiop";
                let uuid="";
                let n = Math.floor(Math.random() * 100) +1;
                for (let i = 0; i < n; i++) {
                    let index = Math.floor(Math.random() * 62);
                    uuid += all.charAt(index);
                }
                return uuid;
            },
        }
